class DropController{
  constructor(){
    ...
    this.barProgressDivEl = document.querySelector('#react-snackbar-root');

    this.barProgressEl = this.barProgressDivEl.querySelector('.mc-progress-bar-fg');

    this.nameFileEl = this.barProgressDivEl.querySelector('.filename');

    this.timeLeftEl = this.barProgressDivEl.querySelector('.timeleft');

    this.fileListEl = document.querySelector('#list-of-files-and-directories');
  ...
  removeFolder(ref,name){
    return new Promise((resolve,reject) =>{
      let folderRef = this.getDbReference(ref + '/'+ name);
  ...
  initEvents(){
    ...
    this.btnNewFolder.addEventListener('click', e =>{
      let name = prompt('what folder name?');

      if(name){
        this.getDbReference().push().set({
          name,
          type: 'folder',
      ...
      this.removeTask().then(responses=> {
        responses.forEach(response =>{
          if(response.fields.key){
            this.getDbReference().child(response.fields.key).remove();
          }
        });
    ...
    this.btnRename.addEventListener('click', e =>{
      ...
      let name = prompt("Renomear arquvio: ", file.name);

      if(name){
        file.name = name;
        this.getDbReference().child(li.dataset.key).set(file);
      }else{
    ...
    this.inputFileEl.addEventListener('change', e =>{
      ...
      this.upFiles(e.target.files).then(responses =>{
        responses.forEach(response =>{
          firebase.storage().ref().child(response.fullPath).getDownloadURL().then(url => {
  ...
  showModal(show = true){
    this.barProgressDivEl.style.display = (show) ? 'block' : 'none';
  }

  initAjax(url, method = 'GET', formData = new FormData(), onprogress = function(){}, onloadstart = 
    function () {} ) {
  ...
  upProgress(event, file){
    ...
    this.barProgressEl.style.width = `${porcent}%`;

    this.nameFileEl.innerHTML = file.name;

    this.timeLeftEl.innerHTML = this.ToSeconds(timeLeft);
  ...
  readFiles(){
    ...
    this.getDbReference().on('value', snapshot =>{
  ...
  openFolder(){
    ...
    if(this.lastFolder) this.getDbReference(this.lastFolder).off('value');
  ...
  eventsLi(li){
    ...
    li.addEventListener('click', (e)=>{
      ...
      if(!e.ctrlKey){
        this.fileListEl.querySelectorAll('li.selected').forEach(el=>{
